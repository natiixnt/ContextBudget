"""Quality-regression corpus for every registered cmd compressor.

Shared by the ``redcon cmd-quality`` / ``redcon cmd-bench`` CLI commands
and by the pytest harness in tests/test_cmd_quality.py. It lives inside
the package (not under tests/) because the CLI commands need it in the
installed wheel; a pip install does not ship the test suite.

Each CASES entry is (name, compressor, raw_stdout, raw_stderr, argv).
Fixtures are generated deterministically so the module is self-contained.
"""
...
def _massive_pytest(num_failures: int = 30, num_passed: int = 200) -> bytes:
    ...
    sections.append("tests/big_suite.py " + ("." * num_passed + "F" * num_failures) + " [100%]\n\n")
    sections.append(
        "=================================== FAILURES ===================================\n"
    )
...
def _massive_find(num_files: int = 500) -> bytes:
    return "\n".join(f"./pkg_{i // 25}/sub_{i // 5}/file_{i}.py" for i in range(num_files)).encode()
...
def _profiler_fixture() -> bytes:
    ...
    for _ in range(200):
        ...
        stack = ";".join(f"{rng.choice(modules)}.{rng.choice(funcs)}" for _ in range(depth))
