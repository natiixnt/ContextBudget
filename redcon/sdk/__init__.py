...
Low-level module functions (quickest path)::
    ...
    from redcon.sdk import prepare_context, simulate_agent, profile_run
...
Class-based SDK (shared config, multi-turn)::
    ...
    sdk = RedconSDK(max_tokens=32_000)
...
from redcon.runtime import AgentRuntime, PreparedContext, RuntimeResult


class RedconSDK:
    """Unified SDK entry point for agent framework integration.

    Wraps :class:`~redcon.engine.BudgetGuard` and
    :class:`~redcon.agents.middleware.RedconMiddleware` behind a
    ...
    def __init__(
        ...
        *,
        ...
        self._middleware = RedconMiddleware(engine=self._guard.engine)
    ...
    def runtime(
        ...
        *,
        ...
        """Create an :class:`~redcon.runtime.AgentRuntime` wired to this SDK's config.
        ...
        policy:
            Optional :class:`~redcon.core.policy.PolicySpec` to
    ...
    def middleware(self) -> RedconMiddleware:
        """Return the underlying :class:`~redcon.agents.middleware.RedconMiddleware`.
...
def prepare_context(
    ...
    *,
    ...
    Equivalent to ``RedconSDK().prepare_context(...)``.  Use this for
    one-off integrations; prefer :class:`RedconSDK` when you need to
    ...
    ::
        ...
        from redcon.sdk import prepare_context
...
def simulate_agent(
    ...
    *,
    ...
    Equivalent to ``RedconSDK().simulate_agent(...)``.
    ...
    ::
        ...
        from redcon.sdk import simulate_agent
        ...
        for step in plan["steps"]:
            print(f"  {step['id']:12}  {step['step_total_tokens']:6} tokens")
    """
    engine = RedconEngine(config_path=config_path)
...
def profile_run(
    ...
    *,
    ...
    Equivalent to ``RedconSDK().profile_run(...)``.
    ...
    ::
        ...
        from redcon.sdk import profile_run
...
def start_gateway(
    ...
    *,
    ...
    """Start the Redcon Runtime Gateway.

    Convenience wrapper that builds a :class:`~redcon.gateway.GatewayConfig`,
    creates a :class:`~redcon.gateway.GatewayServer`, starts it, and
    ...
    config_path:
        Path to a ``redcon.toml`` shared by all gateway requests.
    telemetry_enabled:
    ...
    block:
        ...
        thread and return the :class:`~redcon.gateway.GatewayServer`
        immediately.

    Returns
    -------
    GatewayServer
        The running server instance.  Call :meth:`~redcon.gateway.GatewayServer.stop`
