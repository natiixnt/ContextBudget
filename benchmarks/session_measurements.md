...
| Avg 3-line shingle overlap | 26.7% |
| Avg distinct paths / session | 13.4 |
| Avg repeated path refs / session | 5.8 |
| Avg distinct symbols / session | 109 |
| Avg recurring symbol refs / session | 88 |
| Total raw tokens | 17,938 |
| Total compressed tokens | 7,781 |

## Per-vector verdicts based on session measurements

### V41 path aliases (already shipped)
- 5.8 repeated path mentions per session over 13.4 distinct paths -> 43% of paths show up >=2 times. Per the V41 model the per-call saving is ~6 cl100k tokens per repeat, ~35 tokens / session.

### V42/V43 cross-content dedup
- 3-line shingle overlap across calls in the same session: 26.7%.
- Verdict: SHIP candidate (>=5% cross-call line overlap).

### V49 symbol cards
- 88 recurring symbol references per session over 109 distinct symbols. V49's break-even is per-symbol freq >= 2; 81% of symbols cross the bar. SHIP candidate.
