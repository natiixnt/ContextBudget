...
ContextBudget webhooks normally deliver raw JSON payloads.  These adapters
...
---------------------
CB_CLOUD_SLACK_WEBHOOK_URL        Slack Incoming Webhook URL for policy alerts
CB_CLOUD_PAGERDUTY_ROUTING_KEY    PagerDuty integration routing key (Events API v2)
"""
...
class SlackAdapter:
    """Translates ContextBudget events into Slack Block Kit messages."""
    ...
    def send_policy_violation(self, payload: dict[str, Any]) -> bool:
        ...
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": ":warning: ContextBudget Policy Violation", "emoji": True},
            },
            {
                "type": "section",
                "fields": [
    ...
    def send_budget_overrun(self, payload: dict[str, Any]) -> bool:
        ...
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": ":rotating_light: ContextBudget Budget Overrun", "emoji": True},
            },
            {
                "type": "section",
                "fields": [
    ...
    def send_drift_alert(self, payload: dict[str, Any]) -> bool:
        ...
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": ":chart_with_upwards_trend: ContextBudget Token Drift Alert", "emoji": True},
            },
            {
                "type": "section",
                "fields": [
    ...
    def send_generic(self, event_type: str, payload: dict[str, Any]) -> bool:
        blocks = [
            {
                "type": "section",
                "text": {"type": "mrkdwn", "text": f"*ContextBudget event*: `{event_type}`\n```{json.dumps(payload, indent=2)[:1500]}```"},
            }
        ]
        return self._send(blocks)
...
class PagerDutyAdapter:
    """Translates ContextBudget events into PagerDuty Events API v2 payloads."""
    ...
    def send_policy_violation(self, payload: dict[str, Any]) -> bool:
        ...
        return self._trigger(
            summary=f"ContextBudget policy violation in {repo} (run {run_id})",
            source=repo,
    ...
    def send_budget_overrun(self, payload: dict[str, Any]) -> bool:
        ...
        return self._trigger(
            summary=f"ContextBudget budget overrun in {repo}: {tokens_used:,} > {max_tokens:,} tokens",
            source=repo,
    ...
    def send_drift_alert(self, payload: dict[str, Any]) -> bool:
        ...
        drift_pct = payload.get("token_drift_pct", 0)
        return self._trigger(
            summary=f"ContextBudget token drift +{drift_pct:.1f}% in {repo}",
            source=repo,
    ...
    def send_generic(self, event_type: str, payload: dict[str, Any]) -> bool:
        return self._trigger(
            summary=f"ContextBudget event: {event_type}",
            source=payload.get("repository_id", "contextbudget"),
            severity="info",
            custom_details=payload,
        )
