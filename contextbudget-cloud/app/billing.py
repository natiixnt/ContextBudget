...
"""Stripe billing meter integration for ContextBudget Cloud.

Architecture
------------
ContextBudget uses Stripe Billing Meters (usage-based billing) to report
...
--------------------------------------
CB_CLOUD_STRIPE_SECRET_KEY      Stripe secret API key (sk_live_... / sk_test_...)
CB_CLOUD_STRIPE_METER_ID        Stripe Meter ID for token consumption events
                                  (create with: stripe meters create --event-name cb_tokens_used)
CB_CLOUD_BILLING_ENABLED        Set to "true" to enable billing (default: false)
...
_STRIPE_SECRET_KEY: str = os.getenv("CB_CLOUD_STRIPE_SECRET_KEY", "")
_METER_ID: str = os.getenv("CB_CLOUD_STRIPE_METER_ID", "")
...
async def get_or_create_stripe_customer(pool: Any, org_id: int, org_slug: str) -> str | None:
    ...
    try:
        customer = stripe.Customer.create(
            metadata={"cb_org_id": str(org_id), "cb_org_slug": org_slug},
            description=f"ContextBudget org: {org_slug}",
        )
