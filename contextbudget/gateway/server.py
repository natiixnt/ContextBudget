...
"""ContextBudget Runtime Gateway — FastAPI ASGI service (falls back to stdlib HTTP).
...
def _build_fastapi_app(config: GatewayConfig, handlers: GatewayHandlers):
    ...
    app = FastAPI(title="ContextBudget Gateway", version="1.0.0-alpha", docs_url=None, redoc_url=None)
...
class GatewayServer:
    ...
    Uses FastAPI + Uvicorn when available (install with ``pip install 'contextbudget[gateway]'``),
    ...
    def start(self, *, block: bool = True) -> None:
        ...
        if fastapi_mod is not None:
            ...
            logger.warning(
                ...
                "For production use: pip install 'contextbudget[gateway]'"
            )
    ...
    def _start_fastapi(self, *, block: bool) -> None:
        ...
        logger.info(
            "ContextBudget Gateway (FastAPI) listening on http://%s:%d",
            self._config.host,
            self._config.port,
        )
    ...
    def _start_stdlib(self, *, block: bool) -> None:
        ...
        logger.info(
            "ContextBudget Gateway (stdlib) listening on http://%s:%d",
            config.host,
            config.port,
        )
        if block:
