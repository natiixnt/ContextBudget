...
2. Runs the full ContextBudget optimisation pipeline
...
from contextbudget.core.policy import PolicySpec, load_policy
...
class AgentRuntime:
    ...
    ``agent → ContextBudget → LLM`` architecture.  It wraps
    :class:`~contextbudget.agents.middleware.ContextBudgetMiddleware` with:

    * **Session tracking** — cumulative token and turn history via
      :class:`~contextbudget.runtime.session.RuntimeSession`.
    ...
    * **LLM dispatch** — an optional ``llm_fn`` callable receives the
      ...
      :class:`~contextbudget.runtime.context.RuntimeResult`.
    * **Policy enforcement** — token budget and quality-risk policies are
      evaluated on every turn; ``strict=True`` raises
      :class:`~contextbudget.engine.BudgetPolicyViolationError` on
      violations.
    ...
    policy:
        A :class:`~contextbudget.core.policy.PolicySpec` to evaluate after
        every pack call.
    policy_path:
        Path to a TOML policy file (alternative to passing ``policy``
        directly).
    strict:
        If ``True``, raise :class:`~contextbudget.engine.BudgetPolicyViolationError`
    ...
    session:
        An existing :class:`~contextbudget.runtime.session.RuntimeSession`
    ...
    def __init__(
        ...
        *,
        ...
        self._engine = engine if engine is not None else ContextBudgetEngine(
    ...
    def prepare_context(
        ...
        *,
        ...
        ``agent → ContextBudget → LLM`` pipeline.  Call this when you want
