...
each, producing a reproducible :class:`~contextbudget.core.dataset.DatasetReport`.
"""
...
def build_agent_run_dataset(
    ...
    *,
    ...
    Runs each task through ContextBudget's benchmark pipeline, comparing the
    full-repository baseline against the compressed-pack optimised context, and
    returns an aggregated :class:`~contextbudget.core.dataset.DatasetReport`.
...
def agent_run_dataset_as_dict(
    ...
    *,
    ...
    Returns the standard :func:`~contextbudget.core.dataset.dataset_as_dict`
    ...
    ----------
    report:
        The :class:`~contextbudget.core.dataset.DatasetReport` to serialise.
    task_count:
