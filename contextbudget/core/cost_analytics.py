...
from contextbudget.telemetry.pricing import (
    DEFAULT_MODEL,
    MODEL_PRICING,
    compute_run_costs,
    get_pricing,
...
def _load_observe_entries(paths: list[Path]) -> list[dict[str, Any]]:
    ...
    for path in paths:
        ...
        obs_file = root / ".contextbudget" / "observe-history.json"
...
def _load_history_entries(paths: list[Path]) -> list[dict[str, Any]]:
    """Load entries from .contextbudget/history.json (pack run history)."""
    entries: list[dict[str, Any]] = []
    seen: set[str] = set()
    for path in paths:
        root = path if path.is_dir() else path.parent
        hist_file = root / ".contextbudget" / "history.json"
...
def build_cost_report(
    ...
    *,
    ...
    model:
        Model identifier from :data:`~contextbudget.telemetry.pricing.MODEL_PRICING`
